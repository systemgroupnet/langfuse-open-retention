...
/** Per-project API keys supplied explicitly, as JSON. */
function parseProjectKeys(raw: string | undefined): Record<string, { publicKey: string; secretKey: string }> {
  if (!raw) return {};
  try {
    const parsed = JSON.parse(raw) as Record<string, { publicKey: string; secretKey: string }>;
    for (const [projectId, keys] of Object.entries(parsed)) {
      if (!keys?.publicKey || !keys?.secretKey) {
        throw new Error(`entry for project "${projectId}" needs both publicKey and secretKey`);
      }
    }
    return parsed;
  } catch (e) {
    throw new Error(`LANGFUSE_PROJECT_KEYS is not valid JSON: ${(e as Error).message}`);
  }
}
...
export const config = {
  ...
  langfuse: {
    ...
    /** Organization-scoped key: enables project discovery + auto-provisioning of project keys. */
    ...
    projectKeys: parseProjectKeys(optional("LANGFUSE_PROJECT_KEYS")),
